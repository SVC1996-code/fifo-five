...
export const AI_VERSION = "fifo-ab-1-order-cache";
...
export function chooseMove(
  ...
  const roots = (safe.length ? safe : children)
    .map(child => ({...child, orderingScore: evaluate(child.state,s.turn)}))
  ...
  function visit(
    ...
    const moves = legalMoves(state).map((move) => {
      const child = applyMove(state,move).state;
      return {move,state:child,orderingScore:move === preferred ? 2*WIN : evaluate(child,player)};
    });
    moves.sort((a,b)=>b.orderingScore-a.orderingScore);
    let best = -Infinity,
      pv: number[] = [];
